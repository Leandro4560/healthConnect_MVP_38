...
# Configuración general para Postgres (no se aplicará a sqlite)
ENGINE_KWARGS = {
...
if DATABASE_URL.startswith("sqlite"):
    ...
    ENGINE_CREATION_KWARGS = {}  # no usar pool sizing para sqlite file
else:
...
def create_engine_with_retry(url: str, retries: int = 4, backoff: float = 1.0):
    ...
    for attempt in range(retries):
        try:
            ...
            # Smoke test solo si no es sqlite (sqlite siempre disponible) — usar SELECT 1 para compatibilidad
